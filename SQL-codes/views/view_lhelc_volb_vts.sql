...
AS
WITH 
lhelc_ids AS (SELECT lhelc_id, lhelc_prv_id, lhelc_nxt_id FROM config_data.lh_election) , -- WITH AS lhelc_ids, enlists lower house election configuration IDS 
lh_ids AS (SELECT * FROM lhelc_ids
		RIGHT OUTER JOIN (SELECT ctr_id, lh_id, lhelc_id FROM config_data.lower_house) AS LHS USING (lhelc_id)), -- WITH AS lh_ids, enlists lower house configurations with corresponding elections 
