...
AS
WITH 
configs AS (SELECT ctr_id, sdate, cab_id, uh_id FROM config_data.mv_configuration_events), -- AS configs
...
cab_uh_sts_shr AS (SELECT DISTINCT ON (ctr_id, sdate) ctr_id, sdate, cab_id, uh_id, SUM(pty_uhelc_sts_shr) AS cab_uh_sts_shr
			FROM (SELECT cab_id, pty_id, pty_cab FROM config_data.cabinet_portfolios) AS CAB_PORTFOLIOS
