...
AS
WITH 
configs AS (SELECT ctr_id, sdate, cab_id, prselc_id FROM config_data.mv_configuration_events) , -- AS configs
cab_parties AS (SELECT cab_id, pty_id FROM config_data.cabinet_portfolios WHERE pty_cab IS TRUE), -- AS cab_parties 
...
cab_pty_hos_pty AS (SELECT ctr_id, sdate, ABS(SIGN(pty_id-pty_id_hos)) AS in_cohabitation -- unequals one if party IDs of president and respective cabinet differ 
					FROM config_cab_parties FULL OUTER JOIN
						(SELECT prselc_id, pty_id AS pty_id_hos FROM config_data.presidential_election) AS PTY_ID_HOS
					USING(prselc_id)
