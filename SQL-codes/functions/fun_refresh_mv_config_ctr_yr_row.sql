...
DROP FUNCTION IF EXISTS config_data.refresh_mv_config_ctr_yr_row();
CREATE OR REPLACE FUNCTION config_data.refresh_mv_config_ctr_yr_row() RETURNS VOID AS $$
DECLARE 
	ctr_yr_id RECORD;
BEGIN
	...
	CREATE TABLE temp_difference 
		AS SELECT DISTINCT ON (ctr_id, year, sdate) * 
			...
			WHERE  (ctr_id, year, sdate) 
				NOT IN (SELECT ctr_id, year, sdate FROM config_data.mv_configuration_ctr_yr);

    FOR ctr_yr_id IN SELECT DISTINCT ON (ctr_id, year) ctr_id, year FROM temp_difference
    LOOP
	UPDATE config_data.mv_configuration_ctr_yr 
		SET 
