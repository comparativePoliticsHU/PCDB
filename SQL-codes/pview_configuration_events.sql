CREATE OR REPLACE VIEW view_configuration_events
AS
SELECT DISTINCT *, DATE_PART('year', sdate)::NUMERIC AS year, NULL::DATE AS edate
	FROM
	...
	RIGHT OUTER JOIN
		(SELECT *
			...
			RIGHT OUTER JOIN 
				(SELECT *
					FROM
						(SELECT lh_id, lh_sdate AS sdate, ctr_id 
