	...
	FROM lh_seat_res -- seat results of parties that were not in previous lower house, i.e., new-entry parties
		WHERE  (lh_id, pty_id) NOT IN  -- by exclusion of present stable parties, only new-entry parties to current lower house configuration remain 
			(SELECT DISTINCT ON (CUR_LH.lh_id, CUR_LH.pty_id) CUR_LH.lh_id, CUR_LH.pty_id 
				...
				JOIN (SELECT DISTINCT ON (lh_id, pty_id) lh_seat_res.lh_id, lh_nxt_id, pty_id 
								FROM lh_seat_res LEFT OUTER JOIN lh_ids USING(lh_id)) AS PRV_LH
				ON (CUR_LH.lh_id = PRV_LH.lh_nxt_id AND CUR_LH.pty_id=PRV_LH.pty_id))-- joining current on previous lower house configurations by CUR_LH.lh_id = PRV_LH.lh_nxt_id allows to identify parties that entered both the previous and the current lower, i.e., that present stable parties
...
ret_ptys AS (SELECT DISTINCT ON (lh_id) lh_id, 
		SUM(pty_lh_sts/lh_sts_ttl_computed) OVER (PARTITION BY lh_id) AS ret_ptys_sts_shr
			FROM lh_seat_res -- seat results of parties not in next lower house, i.e., retiring parties
				WHERE  (lh_id, pty_id) NOT IN -- by exclusion of future stable parties, only parties that will retire from current lower house configuration remain
					(SELECT DISTINCT ON (CUR_LH.lh_id, CUR_LH.pty_id) CUR_LH.lh_id, CUR_LH.pty_id 
						FROM (SELECT DISTINCT ON (lh_id, pty_id) lh_id, pty_id FROM lh_seat_res) AS CUR_LH
						JOIN (SELECT DISTINCT ON (lh_id, pty_id) lh_seat_res.lh_id, lh_prv_id, pty_id 
										FROM lh_seat_res LEFT OUTER JOIN lh_ids USING(lh_id)) AS NXT_LH
						ON (CUR_LH.lh_id = NXT_LH.lh_prv_id AND CUR_LH.pty_id=NXT_LH.pty_id))) -- joining current on next lower house configurations by CUR_LH.lh_id = NXT_LH.lh_prv_id allows to identify parties that were in both the current and the next lower house, i.e., future stable parties
SELECT lh_ids.lh_id, 
	ABS(COALESCE(ret_ptys_sts_shr, 0) + COALESCE(new_ptys_sts_shr, 0))/2 AS lh_vola_sts_computed 
FROM lh_ids
LEFT OUTER JOIN new_ptys USING(lh_id)
LEFT OUTER JOIN ret_ptys ON(lh_ids.lh_prv_id = ret_ptys.lh_id) -- joining the set of future retiring parties on lower house configs by lh_prv_id identifies parties that were in last lower house but are not in current lower house configuration, i.e., parties that have acutally retired (from the perspective of the current configuration)
ORDER BY lh_id;