...
matched AS (SELECT ctr_id, year FROM matches WHERE matched = FALSE ), -- WITH as matched		
configs_in_year AS (SELECT ctr_id, year, sdate, edate, 
			...
			SELECT matched.ctr_id as ctr_id, matched.year AS year, max(sdate) AS sdate, max(edate) AS edate, 
				...
				FROM
				max_sdate_in_year_configs AS max_sdate, matched
				WHERE max_sdate.ctr_id = matched.ctr_id
				AND max_sdate.year < matched.year
				GROUP BY matched.year, matched.ctr_id ) , -- WITH as configs_in_year
