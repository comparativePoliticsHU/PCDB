CREATE OR REPLACE VIEW beta_version.view_configuration_duration_weight_in_year
AS 
WITH durations -- computes configurations' duration in the year(s) they were active
	AS (
	...
	-- the following four UNIONs generate a list of configurations such that configurations are listed in all years in which they were active, that is in multiple years if a configuration's end year does not equal its start year
	-- NOTE that the WITH queries defined above are subsequently referenced in FROM-clauses 
...
SELECT ctr_id, sdate, year, duration_in_year, year_duration,
	(duration_in_year::NUMERIC/year_duration::NUMERIC) AS config_weight_in_year
	FROM
		(SELECT ctr_id, sdate, duration_in_year, year
			FROM durations
		) AS DUR_IN_YEAR
	LEFT OUTER JOIN
		(SELECT ctr_id, SUM(duration_in_year) AS year_duration, year
			FROM durations
			GROUP BY ctr_id, year
		) AS YEAR_DUR
	USING(ctr_id, year);
